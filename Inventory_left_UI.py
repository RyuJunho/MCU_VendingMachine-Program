...
class Inventory_left_UI(tk.Tk):
    def __init__(self):
        ...
        self.ChoiceButton = tk.Button(left_frame, text='선택', width=30
                                   , command=lambda: self.action.modify(self.itemTable.focus(),self.itemTable))
    ...
    def choice(self, job_name, treeview):  # 선택버튼시 호출되는 이벤트
        self.action.modify(job_name, treeview)
        self.right_frame.pack(side='right', expand=True)
